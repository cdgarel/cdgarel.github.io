...
var sum = function(array) {
  // base case: run until array.length = 0
  if(array.length === 0) {
    return 0;
  } else {
...
// sumBelow(7); // 21
// if n === 0, return 0
// if n < 0, return sum of array of negative numbers
// return sum of numbers below n up to 0;
...
// if x = y, return empty array
// loop through array to return all numbers between x and y
...
var palindrome = function(string) {
  ...
  if (noWhiteSpace === '' || noWhiteSpace.length === 1) {
    return true; 
    
  } 
  if (noWhiteSpace[0].toLowerCase() !== noWhiteSpace.slice(-1).toLowerCase()) { 
    return false; 
  }
  // recursive case
  return palindrome(noWhiteSpace.substr(1, noWhiteSpace.length - 2));
};
...
var createArray = function(str){
  ...
  .concat(createArray(str.substr(1)));
};
